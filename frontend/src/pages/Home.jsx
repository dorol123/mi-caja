...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 to-emerald-950">
      <div className="max-w-lg mx-auto min-h-screen flex flex-col">
        <div className="px-5 pt-12 pb-6">
          <div className="flex items-center justify-between">
            ...
            <button onClick={logout} className="w-10 h-10 bg-white/15 rounded-full flex items-center justify-center text-white font-bold text-sm border border-white/20">{initials}</button>
          </div>
        </div>
                ...
                <button key={org.id} onClick={() => navigate(`/org/${org.id}`)} className="w-full bg-white rounded-2xl p-5 shadow-xl text-left active:scale-95 transition">
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      {org.reimbursement_balance > 0 && <p className="text-sm font-semibold text-blue-500 mt-2">Te deben {fmt(org.reimbursement_balance)}</p>}
                    </div>
                    <span className={`text-xs font-semibold px-3 py-1.5 rounded-full ml-3 flex-shrink-0 ${org.role === 'admin' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{org.role === 'admin' ? 'Admin' : 'Usuario'}</span>
                  </div>
                </button>
              ))}
            </div>
      ...
      <button onClick={() => setModal('choose')} className="fixed bottom-6 right-5 w-14 h-14 bg-emerald-500 text-white rounded-full shadow-lg shadow-emerald-900/50 flex items-center justify-center active:bg-emerald-600 transition active:scale-95">
