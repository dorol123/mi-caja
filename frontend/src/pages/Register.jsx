...
export default function Register() {
  ...
  const brandSection = (subtitle) => (
    <div className="flex-1 flex flex-col items-center justify-center px-8 z-10 pb-4 pt-14">
      <div className="rounded-[30px] shadow-2xl shadow-emerald-900/60 mb-7"><Logo size={90} /></div>
      <h1 className="text-[2.6rem] font-bold text-white tracking-tight leading-none">Mi Caja</h1>
      <p className="text-emerald-400/60 text-xs mt-2.5 tracking-[0.2em] uppercase font-semibold">{subtitle}</p>
    </div>
  );

  if (step === 2) return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-slate-950 to-emerald-950 relative overflow-hidden">
      <div className="absolute top-24 left-1/2 -translate-x-1/2 w-96 h-96 bg-emerald-500 rounded-full blur-[140px] opacity-[0.18] pointer-events-none" />
      {brandSection('¡Cuenta creada!')}
      <div className="z-10 px-5 pb-10">
        <div className="bg-white rounded-3xl px-6 pt-7 pb-8 shadow-2xl">
          ...
          <form onSubmit={handleName} className="space-y-4">
            <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-[14px] text-slate-800 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:border-transparent text-[15px]" placeholder="Ej: Facundo" autoFocus required />
            {error && (<div className="flex items-center gap-2.5 bg-red-50 border border-red-100 rounded-2xl px-4 py-3"><span className="text-red-400 text-sm flex-shrink-0">⚠</span><p className="text-red-600 text-sm">{error}</p></div>)}
            <button type="submit" disabled={loading} className="w-full bg-emerald-500 text-white rounded-2xl py-4 font-bold text-[15px] shadow-lg shadow-emerald-500/25 active:scale-[0.98] transition-transform disabled:opacity-50">{loading ? 'Guardando…' : 'Continuar'}</button>
          </form>
        </div>
      </div>
    </div>
      ...
      <div className="z-10 px-5 pb-10">
        <div className="bg-white rounded-3xl px-6 pt-7 pb-8 shadow-2xl">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            {error && (<div className="flex items-center gap-2.5 bg-red-50 border border-red-100 rounded-2xl px-4 py-3"><span className="text-red-400 text-sm flex-shrink-0">⚠</span><p className="text-red-600 text-sm">{error}</p></div>)}
            <button type="submit" disabled={loading} className="w-full bg-emerald-500 text-white rounded-2xl py-4 font-bold text-[15px] shadow-lg shadow-emerald-500/25 active:scale-[0.98] transition-transform disabled:opacity-50 mt-1">{loading ? 'Creando cuenta…' : 'Crear cuenta'}</button>
          </form>
          <p className="text-center text-sm text-slate-400 mt-6">¿Ya tenés cuenta?{' '}<Link to="/login" className="text-emerald-600 font-bold">Iniciar sesión</Link></p>
        </div>
      </div>
    </div>
  );
